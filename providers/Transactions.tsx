...
export function TransactionsProvider({ children }: TransactionsProviderProps): JSX.Element {
        ...
        >
          <View style={[ styles.container, { backgroundColor: `${cardColor}`, } ]}>
            ...
            <View style={[ styles.titleContainer ]}>
              <Text style={[ styles.text ]} numberOfLines={1}>{title ? title : "Không có tiêu đề"}</Text>
            </View>
          ...
          >
            ...
            <Text
              style={[
                styles.text,
                {
                  color: `${ type === "Thu tiền" || type === "Tiết kiệm" ? "#70B657" : "#D17777" }`,
                  marginBottom: 20,
                }
              ]}
              numberOfLines={1}
            ...
            <Text style={[ styles.text, { fontFamily: "Poppins-Medium" } ]} numberOfLines={1}>{title ? title : "Không có tiêu đề"}</Text>
            <Text
              style={[
                styles.text,
                {
  ...
  function deleteTransaction({ id, type }: ITransaction): void {
    if(type === "Thu tiền" || type === "Tiết kiệm") {
      setIncomes(prev => prev.filter(item => item.id !== id));
    }

    else if(type === "Chi tiêu" || type === "Khoản vay") {
    ...
    return (
      <>{
        ...
        : <View style={styles.noExpensesContainer}>
          <Text style={[ styles.text, { fontSize: 16 } ]}>Không có chi phí nào.</Text>
        </View>
      }</>
    );
  }
  ...
  async function storeWalletSavings(): Promise<void> {
    ...
    setIncomes(prev => prev.filter(item => item.type !== "Thu tiền"));
    setExpenses(prev => prev.filter(item => item.type !== "Chi tiêu"));
  }
  ...
  function renderToReceiveAndDebts(): void {
    ...
    for(let i = 0; i < incomes.length; i++) {
      if(incomes[i].type === "Tiết kiệm") receiveAndDebts.push(incomes[i]);
    }

    for(let i = 0; i < expenses.length; i++) {
      if(expenses[i].type === "Khoản vay") receiveAndDebts.push(expenses[i]);
    }
  ...
  function renderIncomesAmount(): void {
    ...
    incomes.map(income => {
      income.type === "Thu tiền" ? counter = counter + income.amount : {};
    });
  ...
  function renderExpensesAmount(): void {
    ...
    expenses.map(expense => {
      expense.type === "Chi tiêu" ? counter = counter + expense.amount : {};
    });
  ...
  function renderReceiveAmount(): void {
    ...
    incomes.map(income => {
      income.type === "Tiết kiệm" ? counter = counter + income.amount : {};
    });
  ...
  function renderDebtsAmount(): void {
    ...
    expenses.map(expense => {
      expense.type === "Khoản vay" ? counter = counter + expense.amount : {};
    });
